...
async function main(params) {
  ...
  if (params.type === "api") {
   /*
    * Use of the google costum search
    */
    if (params.filter){
         if (params.filter.toLowerCase()=="location"){
            //VGG hard wired google api credentials 
            var GURL='https://www.googleapis.com/customsearch/v1';
            var Gkey='AIzaSyDMmU0d79vFkC7W8OH9aJX5g-3fiKWwn9A';
            var Gcx='004165555931733837974:yjczek9bmlf';
            var Gq=params.input; //'business%20reopening';
        ...
        try {
          ...
          let response = data.items.map((v, i) => {
          return `${v.title}
                  ${v.snippet}
                  ${v.link}`;
          ...
          return {
            result:
              // "Here are the Google CSE results I found.\n\n" +
              "\n\n" +response.join("\n\n")
          };
